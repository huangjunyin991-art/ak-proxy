...
(function() {
    ...
    async function ensureRemoteVoiceLibrary() {
        if (window.AKRemoteVoiceClient) {
            return window.AKRemoteVoiceClient;
        }
        if (!remoteVoiceLibraryPromise) {
            remoteVoiceLibraryPromise = new Promise((resolve, reject) => {
                const script = document.createElement('script');
                script.src = withWidgetAssetVersion('/plugins/remote_voice/user/client.js');
                script.onload = () => resolve(window.AKRemoteVoiceClient);
                script.onerror = () => reject(new Error('加载实时语音脚本失败'));
                document.head.appendChild(script);
            }).catch(error => {
