...
(function() {
    ...
    function getChatShellModule() {
        try {
            return window.AKClientRuntimeChatShell || null;
        } catch(e) {}
        return null;
    }

    function mountChatShellFallback() {
        ...
        container.innerHTML = `
            ...
            <div id="ak-remote-voice-bar">
                ...
                <button type="button" class="voice-fab-btn" id="ak-remote-voice-mute-btn" onclick="AKChat.toggleVoiceMute()" aria-label="切换麦克风" title="切换本地麦克风">语音</button>
    ...
    function mountChatShell() {
        try {
            const chatShell = getChatShellModule();
            if (chatShell && typeof chatShell.mountShell === 'function') {
                const mounted = chatShell.mountShell();
                if (mounted && mounted.chatBox) return mounted;
            }
        } catch(e) {}
        return mountChatShellFallback();
    }

    const chatShell = mountChatShell();
    const chatBox = chatShell.chatBox;
    const messagesDiv = chatShell.messagesDiv;
    const inputEl = chatShell.inputEl;
    const assistRequestOverlay = chatShell.assistRequestOverlay;
    const assistRequestTitle = chatShell.assistRequestTitle;
    const assistRequestText = chatShell.assistRequestText;
    const remoteVoiceBar = chatShell.remoteVoiceBar;
    const remoteVoicePulse = chatShell.remoteVoicePulse;
    const remoteVoiceMuteBtn = chatShell.remoteVoiceMuteBtn;
    const remoteVoiceAudio = chatShell.remoteVoiceAudio;
    ...
    function getChatUiModule() {
        try {
            return window.AKClientRuntimeChatUI || null;
        } catch(e) {}
        return null;
    }
    ...
    function playNotificationSound() {
        try {
            ...
            if (chatUi && typeof chatUi.playNotificationSound === 'function') {
                chatUi.playNotificationSound();
                return;
            }
        } catch(e) {}
        try {
    ...
    function escapeHtml(text) {
        try {
            const chatUi = getChatUiModule();
            if (chatUi && typeof chatUi.escapeHtml === 'function') {
                return chatUi.escapeHtml(text);
            }
        } catch(e) {}
