(function() {
    ...
    function injectStyle() {
        ...
        style.textContent = [
            ...
            '.ri-row-actions{display:flex;align-items:center;gap:8px;flex-wrap:nowrap}.ri-row-actions .ri-btn{flex:0 0 auto}',
            '#riskIsolationPanelMount .ri-toolbar .ri-btn,#riskIsolationPanelMount .ri-row-actions .ri-btn{border:1px solid rgba(140,198,230,.26)!important;font-weight:900!important;transition:transform .16s ease,filter .16s ease,border-color .16s ease!important;background:rgba(7,20,37,.76)!important;box-shadow:none!important}#riskIsolationPanelMount #riIsolateAllBtn{color:#ffc9cf!important;background:linear-gradient(135deg,rgba(255,107,107,.24),rgba(230,57,70,.1))!important;border-color:rgba(255,107,107,.48)!important;box-shadow:0 8px 18px rgba(230,57,70,.1)!important}#riskIsolationPanelMount #riReleaseScopeBtn{color:#9bf7ef!important;background:linear-gradient(135deg,rgba(0,212,255,.2),rgba(0,255,136,.1))!important;border-color:rgba(0,212,255,.48)!important;box-shadow:0 8px 18px rgba(0,212,255,.1)!important}#riskIsolationPanelMount .ri-row-actions .ri-btn{min-height:34px!important;border-radius:9px!important;padding:0 11px!important;box-shadow:none!important}#riskIsolationPanelMount .ri-row-actions button[data-ri-action="isolate"]{color:#ffc9cf!important;background:linear-gradient(135deg,rgba(255,107,107,.18),rgba(230,57,70,.08))!important;border-color:rgba(255,107,107,.4)!important}#riskIsolationPanelMount .ri-row-actions button[data-ri-action="isolateUmbrella"]{color:#ffd992!important;background:linear-gradient(135deg,rgba(255,209,102,.18),rgba(248,150,30,.08))!important;border-color:rgba(255,209,102,.42)!important}#riskIsolationPanelMount .ri-row-actions button[data-ri-action="release"],#riskIsolationPanelMount .ri-row-actions button[data-ri-action="releaseUmbrella"]{color:#9bf7ef!important;background:linear-gradient(135deg,rgba(0,212,255,.16),rgba(0,255,136,.07))!important;border-color:rgba(0,212,255,.38)!important}#riskIsolationPanelMount .ri-toolbar .ri-btn:not(:disabled):hover,#riskIsolationPanelMount .ri-row-actions .ri-btn:not(:disabled):hover{filter:brightness(1.13);transform:translateY(-1px)}',
    ...
    function buildShell() {
        ...
        root.innerHTML = '<div class="ri-wrap">' +
            ...
            '<div class="ri-modal" id="riActionModal"><div class="ri-modal-card"><div class="ri-modal-head"><div><div class="ri-modal-title" id="riModalTitle">风险隔离确认</div><div class="ri-modal-desc" id="riModalDesc"></div></div></div><div class="ri-modal-body"><div class="ri-modal-input-wrap" id="riModalInputWrap"><label class="ri-modal-label" for="riModalReason">隔离原因（可选）</label><textarea class="ri-modal-input" id="riModalReason" rows="3" placeholder="填写原因，便于后续审计追踪"></textarea></div></div><div class="ri-modal-foot"><button type="button" class="ri-modal-btn cancel" id="riModalCancel">取消</button><button type="button" class="ri-modal-btn confirm" id="riModalConfirm">确定</button></div></div></div>' +
        '</div>';
        bindEvents();
    ...
    function setBusy(busy) {
        ...
        Array.prototype.slice.call(document.querySelectorAll('#riskIsolationPanelMount [data-ri-action]')).forEach(function(el) {
            el.disabled = !!busy;
        });
        var page404Switch = document.getElementById('riPage404Switch');
    ...
    function showRiskIsolationModal(config) {
        return new Promise(function(resolve) {
            ...
            if (!modal || !title || !desc || !inputWrap || !reasonInput || !cancelBtn || !confirmBtn) {
    ...
    function isolateUmbrella(username) {
        return showRiskIsolationModal({
            title: '隔离伞下玩家',
            desc: '确认隔离 [' + username + '] 组织架构下全部成员吗？包含该玩家本人；如果尚未缓存组织架构，系统会先自动获取，成员较多时请等待完成。',
            confirmText: '确认伞下隔离',
            reason: true
        }).then(function(result) {
            if (!result.confirmed) return;
            setBusy(true);
            return apiPost('/isolate_umbrella', Object.assign(scopePayload(), { account: username, reason: result.reason })).then(function(data) {
                notify(data.message || '已隔离伞下玩家');
                return loadStatus().then(loadAccounts);
            }).catch(function(err) {
                notify(err.message || '伞下隔离失败', 'error');
            }).finally(function() {
                setBusy(false);
            });
        });
