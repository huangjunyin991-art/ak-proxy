(function() {
    ...
    function renderAccountRows(accounts, jobs) {
        ...
        return rows.map(function(account) {
            ...
            var status = done ? '已完成' : (pending ? '离线保护' : '待扫描');
            return '<tr>' +
        ...
        return '' +
            '<div class="ak-gss-root">' +
                ...
                '<section class="ak-gss-metrics">' +
                    ...
                    '<div><span>被指导账号数量</span><strong>' + number(summary.matched_subaccounts) + '</strong></div>' +
                '</section>' +
                '<section class="ak-gss-section">' +
                    ...
                    '<div class="ak-gss-table-wrap"><table><thead><tr><th>白名单账号</th><th>状态</th><th>被指导账号数量</th></tr></thead><tbody>' + renderAccountRows(data.accounts, data.jobs) + '</tbody></table></div>' +
                '</section>' +
                '<section class="ak-gss-section">' +
