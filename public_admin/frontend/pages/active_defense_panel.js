(function() {
    ...
    function renderStyles() {
        return '<style>' + [
            ...
            '.ad-policy-stack{display:flex;flex-direction:column;gap:14px}',
            '.ad-policy-group{border:1px solid rgba(0,212,255,.18);border-radius:18px;background:rgba(0,212,255,.035);padding:12px}',
            '.ad-policy-group-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba(255,255,255,.07)}',
            '.ad-policy-group-title{color:var(--text-primary);font-size:14px;font-weight:900}.ad-policy-group-desc{color:var(--text-secondary);font-size:11px;line-height:1.45;margin-top:4px}',
            '.ad-policy-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.ad-policy-fields.three{grid-template-columns:repeat(3,minmax(0,1fr))}',
    ...
    function policyGroup(title, desc, controls, columns) {
        return '<div class="ad-policy-group"><div class="ad-policy-group-head"><div><div class="ad-policy-group-title">' + escapeHtml(title) + '</div><div class="ad-policy-group-desc">' + escapeHtml(desc) + '</div></div></div><div class="ad-policy-fields ' + (columns === 3 ? 'three' : '') + '">' + controls + '</div></div>';
    }

    function section(title, copy, index, body, wide) {
