...
NOTIFY_CENTER_IDENTITY_COOKIE="${NOTIFY_CENTER_IDENTITY_COOKIE:-ak_notify_identity}"
NOTIFY_CENTER_ENABLED="${IM_NOTIFY_CENTER_ENABLED:-${NOTIFY_CENTER_ENABLED:-}}"
IM_ALLOW_UNSIGNED_IDENTITY="${IM_ALLOW_UNSIGNED_IDENTITY:-0}"
...
case "$IM_ALLOW_UNSIGNED_IDENTITY" in
    1|true|TRUE|yes|YES|on|ON|enabled|ENABLED)
        echo "[WARN] IM_ALLOW_UNSIGNED_IDENTITY 已启用，仅应用于本地开发环境"
        ;;
    *)
        if [ -z "$NOTIFY_CENTER_IDENTITY_SECRET" ]; then
            echo "[ERROR] 缺少 NOTIFY_CENTER_IDENTITY_SECRET 或 IM_NOTIFY_CENTER_WEBHOOK_SECRET，IM 生产环境不能回退明文 Cookie 身份"
            exit 1
        fi
        ;;
esac
