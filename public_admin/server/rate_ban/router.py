...
def create_rate_ban_router(
    config_service: RateBanConfigService,
    verify_admin_token: Callable[[str], object],
    get_token_role: Callable[[str], str],
    super_admin_role: str,
) -> APIRouter:
    router = APIRouter(prefix="/admin/api/rate-ban")
    ...
    async def require_super_admin(request: Request):
        ...
        if get_token_role(token) != super_admin_role:
            return None, JSONResponse(status_code=403, content={"error": True, "message": "仅系统总管理员可配置限速封禁"})
        return token, None
    ...
    async def clear_runtime(request: Request):
        ...
        try:
            ...
            if not result.get("success"):
                return JSONResponse(status_code=503, content={"error": True, "message": result.get("message") or "限速封禁模块不可用"})
            return result
        except Exception as exc:
            return JSONResponse(status_code=500, content={"error": True, "message": str(exc)[:300]})
