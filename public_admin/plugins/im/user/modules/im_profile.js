     ...
     const profileModule = {
         ...
         renderProfileSettingsView(container, displayName, honorName, username, nickname, genderLabel, profile) {
            ...
            '</div>' +
             ...
             const copyWithHint = typeof this.ctx.copyWithHint === 'function' ? this.ctx.copyWithHint : null;
             const ntfyTopicInput = container.querySelector('#ak-im-profile-ntfy-topic');
             const ntfyTopicBtn = container.querySelector('[data-ak-copy-btn="ntfy-topic"]');
             ...
             if (ntfyTopicInput && copyWithHint) {
                 ntfyTopicInput.addEventListener('click', bindCopyTopic);
             }
             if (ntfyTopicBtn && copyWithHint) {
                 ntfyTopicBtn.addEventListener('click', function(ev) { try { ev.preventDefault(); ev.stopPropagation(); } catch (e) {} bindCopyTopic(); });
             }

             const ntfyServerInput = container.querySelector('#ak-im-profile-ntfy-server');
             const ntfyServerBtn = container.querySelector('[data-ak-copy-btn="ntfy-server"]');
             ...
             if (ntfyServerInput && copyWithHint) {
                 ntfyServerInput.addEventListener('click', bindCopyServer);
             }
             if (ntfyServerBtn && copyWithHint) {
                 ntfyServerBtn.addEventListener('click', function(ev) { try { ev.preventDefault(); ev.stopPropagation(); } catch (e) {} bindCopyServer(); });
             }
         }
     };
