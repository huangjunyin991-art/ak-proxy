...
var wsBase=/^wss?:\/\//.test(cfg.wsEndpoint)?cfg.wsEndpoint:(proto+location.host+cfg.wsEndpoint);
var ws=null;
var overlay=null;
var clearTimer=0;
function adminHeaders(){var token='';try{token=sessionStorage.getItem('admin_token')||'';}catch(_e){}return token?{'Authorization':'Bearer '+token}:{};}
function buildTicketUrl(ticket){var url=new URL(wsBase,location.origin);url.protocol=location.protocol==='https:'?'wss:':'ws:';url.search='';url.searchParams.set('ticket',String(ticket||''));return url.toString();}
function fetchTicket(){var role=String(cfg.role||'user').toLowerCase();var endpoint=role==='admin'?'/admin/api/ws-ticket':'/chat/api/ws-ticket';var headers={'Content-Type':'application/json'};if(role==='admin'){var admin=adminHeaders();Object.keys(admin).forEach(function(key){headers[key]=admin[key];});}return fetch(endpoint,{method:'POST',credentials:'include',headers:headers,body:JSON.stringify({audience:'assist',session_id:cfg.sessionId,role:role,site:cfg.site||'ak_web',readonly:!!cfg.readonly})}).then(function(resp){return resp.json().catch(function(){return {};}).then(function(data){if(!resp.ok||!data||!data.ticket){throw new Error((data&&data.message)||('ws ticket failed: '+resp.status));}return data.ticket;});});}
...
function connect(){fetchTicket().then(function(ticket){try{ws=new WebSocket(buildTicketUrl(ticket));}catch(_e){setTimeout(connect,1500);return;}ws.onopen=function(){send('client_hello',{role:cfg.role||'user',readonly:!!cfg.readonly,capabilities:['route_sync','click_highlight']});if(cfg.role!=='admin'){emitRoute();}heartbeat();};ws.onmessage=function(event){try{handleMessage(JSON.parse(event.data||'{}'));}catch(_e){}};ws.onerror=function(){};ws.onclose=function(){setTimeout(connect,1500);};}).catch(function(){setTimeout(connect,1500);});}
