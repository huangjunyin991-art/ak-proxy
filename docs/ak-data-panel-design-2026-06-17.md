...
- 成交量。
- 成交价值。
- 手续费扣除：`readonly_stock_count - mycancel - success`。
- 唯一卖家数。
- 唯一买家数。
- `FlowNumber=0` 订单数。
- 最近订单 ID。
...
- AK交易统计：柱图只堆叠显示成交量、交易销毁、手续费扣除；tooltip 完整显示挂卖量、成交量、交易销毁、手续费扣除。
- AK成交统计：成交价值、成交价格。
- 买家数/卖家数趋势。
- `FlowNumber=0` 订单占比。
...
管理员面板需要提供通过卖家或买家 `FlowNumber` 查询关联订单的功能。这个入口面向业务核验，不以订单 ID 作为主要查询条件。
...
点击关联订单行后，表格切换为该订单的买家明细视图。前 3 列保持订单上下文，后续列收缩为买家数据，避免在同一行内塞过多字段：
...
从买家明细视图返回订单列表时，不重新请求上游，只恢复当前查询结果或最近订单列表。
...
- 总管理员和有权限的子管理员都可查询本地数据。
- 查询接口只读，不触发上游请求。

这个功能用于账户维度核验，不参与自动扫描队列，避免管理员查询时影响全局采集进度。订单 ID 仍作为明细字段展示，方便必要时回查单笔订单。
